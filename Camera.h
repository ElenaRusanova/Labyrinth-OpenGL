...
public:
    ...
    float jumpSpeed = 5.0f;     // начальная скорость прыжка
    float gravity = -9.81f;     // сила гравитации
    bool isJumping = false;     // флаг для проверки, находимся ли мы в прыжке
    float verticalVelocity = 0.0f; // текущая вертикальная скорость

    void UpdateJump(float deltaTime) {
        if (isJumping) {
            verticalVelocity += gravity * deltaTime;  // учитываем гравитацию
            Position.y += verticalVelocity * deltaTime;

            // Когда камера достигла земли, останавливаем прыжок
            if (Position.y <= 0.0f) {
                Position.y = 0.0f;
                isJumping = false;
    ...
    void Jump() {
        if (!isJumping) {
            isJumping = true;
            verticalVelocity = jumpSpeed;  // задаем начальную скорость вверх
        }
    }
    // КОНЕЦ ПРЫЖКА
